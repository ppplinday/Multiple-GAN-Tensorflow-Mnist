...
class WGAN:
	...
	def build(self):
		...
		self.D = self.d(self.images)
		self.D_ = self.d(self.G)
	...
	def train(self):
		...
		for id in range(1000001):
			...
			# update D and G
			if id % 5 == 0:
				_, summary_str, err_d, _ = self.sess.run([d_optim, self.d_sum, self.d_loss, self.clip_D], 
